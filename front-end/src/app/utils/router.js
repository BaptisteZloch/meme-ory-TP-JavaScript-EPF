...
export class Router {
  ...
  register(url, componentCtor) {
    if (
      !componentCtor ||
      !isFunction((componentCtor.prototype || {}).getTemplate)
    ) {
      throw new TypeError(
        `provided arg should be a Component. Got: ${get(
          componentCtor,
  ...
  async _renderComponent(componentCtor) {
    ...
    if (oldRenderId === getRenderId()) {
      throw new Error(
        `Error while navigating to route ${route}: Component.render() not called. Did you forgot to call super.render()?`
      );
    }
